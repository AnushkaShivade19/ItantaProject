"""
Backend Coder Agent — TDD-pass implementation generator.
...
SYSTEM_PROMPT = """You are the Backend Coder Agent in a TDD-first AI software-generation pipeline.
...
- If you are writing Javascript/React (.js, .jsx), you MUST write modern, idiomatic React functional components.
...
class BackendCoderAgent(BaseAgent):
    ...
    async def execute(self, run_id: str, **kwargs: Any) -> dict[str, Any]:  # noqa: ARG002
        ...
        if not run.tasks:
            raise RuntimeError("backend_coder requires planner tasks")
        ...
        for task in run.tasks:
            if getattr(task, "code_file_paths", None):
                # If code files were already generated for this task, skip to save tokens
        ...
        summary = f"backend_coder: {total_files} files across {len(run.tasks)} tasks"
    ...
    def _persist_files(
        ...
        if not isinstance(files, list):
            raise RuntimeError(f"backend_coder: invalid files payload for task {task.id}")
        ...
        if written:
            # We append the code file paths rather than overwriting so that Designer's files aren't lost
            run = self.state.get(run_id)
            current_files = []
            for t in run.tasks:
                if t.id == task.id:
                    current_files = list(t.code_file_paths)
                    break
            self.state.set_task_code_files(run_id, task.id, current_files + written)
